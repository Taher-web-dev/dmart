...
curl -s -H "$AUTH" -H "$CT" -d "$REQUEST" ${API_URL}/managed/request | jq .status 
...
curl -s -H "$AUTH" -H "$CT" -d "$RECORD" ${API_URL}/managed/request | jq .status 

echo -n -e "Create Schema: \t\t"
curl -s -H "$AUTH" -F 'space_name="demo"' -F 'request_record=@"../spaces/demo/test/createschema.json"' -F 'payload_file=@"../spaces/demo/test/schema.json"' ${API_URL}/managed/resource_with_payload | jq .status
 
echo -n -e "Create content: \t"
curl -s -H "$AUTH" -F 'space_name="demo"' -F 'request_record=@"../spaces/demo/test/createcontent.json"' -F 'payload_file=@"../spaces/demo/test/data.json"' ${API_URL}/managed/resource_with_payload  | jq  .status

echo -n -e "Upload attachment: \t"
curl -s -H "$AUTH" -F 'space_name="demo"' -F 'request_record=@"../spaces/demo/test/createmedia.json"' -F 'payload_file=@"../spaces/demo/test/logo.jpeg"' ${API_URL}/managed/resource_with_payload  | jq .status
