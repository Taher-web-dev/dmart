...
def get_redis_index_fields(key_chain, property, redis_schema_definition):
    ...
    for property_key, property_value in property["properties"].items():
        redis_schema_definition = get_redis_index_fields(
        ...
        for schema_path in path.glob(schemas_glob):
            ...
            for key, property in schema_content["properties"].items():
                redis_schema_definition.extend(
                    get_redis_index_fields(key, property, [])
                )

            if redis_schema_definition:
                redis_indices[space_name][schema_shortname] = client.ft(
                ...
                redis_schema_definition.extend(
                    [
                        ...
                        TextField(
                            "$.resource_type", no_stem=True, as_name="resource_type"
                        ),
                ...
                create_index(
                    space_name, schema_shortname, tuple(redis_schema_definition)
                )


def generate_doc_id(
    space_name: str, schema_shortname: str, shortname: str, subpath: str
):
    return f"{space_name}:{schema_shortname}:{subpath}/{shortname}"


def save_meta_doc(
    space_name: str, schema_shortname: str, subpath: str, meta: core.Meta
):
